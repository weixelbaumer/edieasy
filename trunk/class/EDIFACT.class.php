<?php
	...
	class EDIFACT {
		...
		var $TABLE_BUILD_MESSAGE = "";
		
		function CreatSegment($NameSegement,$Groupe,$OccurenceGroupe,$IdNameSegement) {
		}
		
		function WriteDataSegment($DataName,$Segment,$IdNameSegement) {
		}
		...
		function GetDataSegment($DataList) {
			...
			$Element = $donnees[2];
			$Name = $this->NameSegment($this->VALEURSEGMENT);
			...
			if (strcmp($key,$Element) === 0) {
				...
				if (strcmp($Name,$donnees[0]) === 0) {
					$Valeur0 = $this->ExtractElement($this->VALEURSEGMENT,$donnees[1]);
				} else {
					$Valeur0 = @$this->ExtractElement($this->TABLE_SEGMENT[$donnees[0]]["VALEUR"],$donnees[1]);
				}
		...
		function LoadSegment($Segment) {
			$Version = $this->TABLE_MESSAGE["VERSION"].$this->TABLE_MESSAGE["RELEASE"];
		...
		function AddMessage($SegmentNumber,$Type,$Valeur,$Mandatory,$Mini,$Maxi,$Level) {
			$this->TABLE_MESSAGE["MAXI_INDEX"] = $SegmentNumber;
			$this->TABLE_MESSAGE[$SegmentNumber]=array("TYPE"=>$Type,"VALEUR"=>$Valeur,"MANDATORY"=>$Mandatory,"MINI"=>$Mini,"MAXI"=>$Maxi,"LEVEL"=>$Level,"OCCURENCE"=>0);
		...
		function IndexMessage($NumSegment,$ValeurSegment) {
			...
			$INDEXED = TRUE;
			$FIND = FALSE;
			while (($FIND === FALSE) && ($INDEXED === TRUE)) {
				if ($this->CurrentIndexMessage > $this->TABLE_MESSAGE["MAXI_INDEX"]) {
					$INDEXED = FALSE; 
				} else {
				$Test = $this->TABLE_MESSAGE[$this->CurrentIndexMessage]["VALEUR"];
				$Type = $this->TABLE_MESSAGE[$this->CurrentIndexMessage]["TYPE"];
				$Maxi = $this->TABLE_MESSAGE[$this->CurrentIndexMessage]["MAXI"];
				$Mandatory = $this->TABLE_MESSAGE[$this->CurrentIndexMessage]["MANDATORY"];
				if ($Test === $Name) {
					...
					$this->TABLE_INDEXATION[$NumSegment]["LEVEL"] = $this->TABLE_MESSAGE[$this->CurrentIndexMessage]["LEVEL"];
					...
					if ($this->TABLE_INDEXATION[$NumSegment]["OCCURENCE"] > $Maxi) {
						$TextErreur = "ERREUR : OCCURENCE SURNUMERAIRE (".$Test." - MAXI:".$Maxi." - EXIST:".$this->TABLE_INDEXATION[$NumSegment]["OCCURENCE"].")<br>";
						$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur;
						$INDEXED = FALSE;
					}
					$FIND = TRUE;
				} else {
					...
					if ($Type === "GROUP") {
						...
						$TestNextSeg = $this->TABLE_MESSAGE[$NextIndexMessage]["VALEUR"];
						if ($TestNextSeg === $Name) {
							...
							while ($Test <> $TestNextSeg) {
								$Indice++;
								$NextIndexMessage = $Indice;
								if ($NextIndexMessage > $this->TABLE_MESSAGE["MAXI_INDEX"]) {
									$TestNextSeg = $Test;
								} else {
									$TestNextSeg = $this->TABLE_MESSAGE[$NextIndexMessage]["VALEUR"];
									$Groupe = $this->TABLE_MESSAGE[$NextIndexMessage]["TYPE"];
							...
							if ($this->TABLE_MESSAGE[$NextIndexMessage]["LEVEL"] === 0) {
								$this->CurrentGroupMessage = 0;
							}
						...
						if ($Type === "ENDGROUP")  {
							$Indice--;
							$TestPrevSeg = $this->TABLE_MESSAGE[$Indice]["VALEUR"];
							...
							if (strcmp($Mandatory,"M") === 0) {
								$PrevNumSegment = $NumSegment-1;
								if ($PrevNumSegment > 0) {
									$PrevSegment = $this->TABLE_INDEXATION[$PrevNumSegment]["VALEUR"];
									if (strcmp($PrevSegment,$Test) === 1) {
										$TextErreur = "ERREUR : Segment MANDATORY not present!!! (0".$PrevNumSegment."0 - ".$Test." - ".$Type." - ".$Mandatory." - ".$Maxi.")<br>";
										$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur;
										$INDEXED = FALSE; 
									} else {
										if ($this->CurrentIndexMessage === $this->TABLE_MESSAGE["MAXI_INDEX"]) {
											$TextErreur = "ERREUR 3 : Current Index Message > MAXI_LINE (".$this->CurrentIndexMessage." > ".$this->TABLE_MESSAGE["MAXI_INDEX"].")<br>";
											$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur;
											$INDEXED = FALSE; 
										} else {
											$this->CurrentIndexMessage++;
									...
									if ($this->CurrentIndexMessage === $this->TABLE_MESSAGE["MAXI_INDEX"]) {
										$TextErreur = "ERREUR 4 : Current Index Message > MAXI_LINE (".$this->CurrentIndexMessage." > ".$this->TABLE_MESSAGE["MAXI_INDEX"].")<br>";
										$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur; 
										$INDEXED = FALSE; 
									} else {
										$this->CurrentIndexMessage++;
								...
								if ($this->CurrentIndexMessage === $this->TABLE_MESSAGE["MAXI_INDEX"]) {
									$TextErreur = "ERREUR 5 : Current Index Message > MAXI_LINE (".$this->CurrentIndexMessage." > ".$this->TABLE_MESSAGE["MAXI_INDEX"].")<br>";
									$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur;
									$INDEXED = FALSE; 
								} else {
									$this->CurrentIndexMessage++;
				...
				if (($INDEXED === TRUE) && ($FIND === FALSE) && ($this->CurrentIndexMessage > $this->TABLE_MESSAGE["MAXI_INDEX"])) {
					$TextErreur = "ERREUR 2 : Current Index Message > MAXI_LINE (".$this->CurrentIndexMessage." > ".$this->TABLE_MESSAGE["MAXI_INDEX"].")".$INDEXED.$FIND."<br>";
					$this->TABLE_INTERCHANGE["ERROR_TEXT"]= $TextErreur;
					$INDEXED = FALSE; 
				}
			}
		...
		function DisplayIndexationMessage() {
			...
			while ($NumSegment <= $this->TABLE_INDEXATION["MAXI_LINE"]) {
				...
				echo "<TD>".@$this->TABLE_INDEXATION[$NumSegment]["INDEX_MESSAGE"]."(".@$this->TABLE_MESSAGE[$this->TABLE_INDEXATION[$NumSegment]["INDEX_MESSAGE"]]["VALEUR"].")</TD>";
		...
		function LoadMessage($Name,$Version,$Release) {
			...
			$this->TABLE_MESSAGE = array("NAME"=>$Name,"VERSION"=>$Version,"RELEASE"=>$Release);
									...
									if (strcmp($Data,"_EOF_") === 0) {
										...
										if ($Test === FALSE) {
										   $this->DisplaySegment($this->VALEURSEGMENT);
											...
											$Test = $this->IndexMessage($NumSegment,$Data);
											if ($Test === FALSE) {
												echo "<H2><BR>".$this->TABLE_INTERCHANGE["ERROR_TEXT"]."</H2><BR>";
												$this->DisplayIndexationMessage();
												$LECTURE = FALSE;
											} else {
												$Name = $this->NameSegment($Data);
												if (strcmp($Name,"UNT") === 0) {
		...
		function IsCurrentGroupName($GroupName) {
			$Test = FALSE;
			$CurrentGroupeName =  @$this->TABLE_INDEXATION[$this->PointeurMessage]["GROUPE"];
			if (strcmp($GroupName,$CurrentGroupeName) === 0) {
				$Test = TRUE;
			}
			return $Test;
		}
		...
		// Get all filtre information for work on the Current Segment
		function IsCurrentGroupSegment($GroupeName,$SegmentName) {
			$Test = FALSE;
			$CurrentGroupeName  = self::GetCurrentGroupName();
			$CurrentSegmentName = self::GetCurrentSegmentName();
			if (strcmp($GroupeName,$CurrentGroupeName) === 0) {
				if (strcmp($SegmentName,$CurrentSegmentName) === 0) {
					$Test = TRUE;
				}
			}
		...
		function PrintOutputFile($OutputFile) {
			...
			while ($i <= $Max) {
				...
				echo $this->OUTPUT[$OutputFile]["DATA"]."<BR>";
				$i++;
			}
		}
		...
		function DisplayClassEdifactInformation() {
			...
			$FileEDIFACTinfo = _PATH_."class\\EDIFACT.class.info";
			$IdFileEDIFACTinfo = fopen($FileEDIFACTinfo,"r");
			
			while ($Info = fgets($IdFileEDIFACTinfo)) {
					echo $Info;
			}
		}
	}
